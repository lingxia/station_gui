...
class STAFer():
    ...
    def station_off(self,Tomachine,command):
        ...
        try:
            ...
            return e.rc
        return 0
    def list_activestation(self,Tomachine,station_type):
        # 1:build
        # 2:run
        # return mac list
        yourTestMap = []
        ...
        try:
            handle = STAFHandle("STAF_SERVICE")
            request = "stat "  + 'type ' + str(station_type)
            result = handle.submit(Tomachine, "mcuscheduler", request)
            mc = unmarshall(result.result)
            yourTestMap = mc.getRootObject()
            if (result.rc != 0):
                return result.rc,yourTestMap
            rc = handle.unregister()
        except STAFException, e:
            return e.rc,yourTestMap
        for station_detail in yourTestMap:
            mac_list.append(station_detail['mac'])
        return 0,mac_list
