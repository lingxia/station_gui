...
def Get_DP_info():
    ...
    '''
    try:
        key = _winreg.OpenKey(_winreg.HKEY_LOCAL_MACHINE,r"SOFTWARE\Freescale\DaPeng Client")
        dp_path,type = _winreg.QueryValueEx(key,"InstallLocation")
    except Exception,e:
        dp_info = {'exist':'no'}
    else:
        dp_info = {'exist':'yes','path':dp_path}
    return dp_info
def Get_Gcc_info(gcc_dir):
    '''
    return gcc version
    '''
    gcc = gcc_dir + '/bin/arm-none-eabi-gcc.exe'
    if os.path.isfile(gcc):
        cmd = gcc + ' -v'
        cur_dir = os.getcwd()
        os.chdir(gcc_dir+'/bin/')
        try:
            output = subprocess.check_output(['arm-none-eabi-gcc.exe','--version'])
