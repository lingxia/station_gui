...
import station_map
def send_result(host,user,pwd, main_dir, port,directory,remote_dir,sof,local_bin,remote_bin,needrun):
    ...
    while i<3:
        try:
            ...
            if sof == 0 and needrun == 1:# send bin only when task finished sucessfully and this case needrun
                logging.info('Starting send binary to Ftp server')
                f.upload_file1(local_bin,remote_bin)
            i = 3
        except Exception,e:
            i = i +1
...
def notify_scheduler_build(Tomachine,Execute_Result,request_id,remote_bin):
    # 1:buildresult 2: runresult . 1:pass 2:fail  8:update_fail
    if Execute_Result == 0:
        Execute_Result = 'pass'
    elif Execute_Result == 1:
        Execute_Result = 'fail'
    elif Execute_Result == 8:
        Execute_Result = 'update_fail'
    else:
        pass
    cmd = 'TASKID ' + request_id + ' RESULTTYPE ' + '1' + ' TESTRESULT ' + station_map.to_scheduler_map[Execute_Result] + ' BINARY ' + remote_bin
